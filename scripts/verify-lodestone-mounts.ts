...
import {
  ...
  getLodestoneMountOwnership,
  lodestoneMountServiceConfig,
} from '../src/lib/lodestone/lodestoneMountService.ts'
import { getCollectionDictionaryEntries } from '../src/lib/lodestone/collectionDictionary.ts'

const __dirname = dirname(fileURLToPath(import.meta.url))
const OUTPUT_PATH = resolve(__dirname, 'output/lodestone-mounts-verify.json')

const SAMPLE_ID = process.argv[2] ?? '60814884'
const EXPECTED_DICTIONARY_TOTAL = getCollectionDictionaryEntries('マウント').length
...
async function main() {
  ...
  const result = await getLodestoneMountOwnership(SAMPLE_ID, 'verify-mounts')
  ...
  if (!result.ok) {
    ...
    if (ownership.totalOnLodestone < 28 || ownership.totalOnLodestone > 38) {
      failures.push(`totalOnLodestone expected ~33, got ${ownership.totalOnLodestone}`)
    }
    ...
    if (ownership.owned.length === 0) {
      failures.push('expected at least one owned mount')
    }
    ...
    const cachedResult = await getLodestoneMountOwnership(SAMPLE_ID, 'verify-mounts-cache')
  ...
  const report = {
    ...
    sampleId: SAMPLE_ID,
    config: lodestoneMountServiceConfig,
